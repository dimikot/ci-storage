...
from helpers import AsgHandler, AsgSpec, Runner, RunnersRegistry, ExpiringDict, log
...
class HandlerIdleRunners(AsgHandler):
    ...
    def handle(self, runners: list[Runner]) -> None:
        ...
        for runner in old_idle_runners[min_size:]:
            has_aws = aws_autoscaling_terminate_instance(
                instance_id=runner.instance_id(),
                should_decrement_desired_capacity=True,
            )
            log(
                f"terminating old idle instance {runner.name} in {self.asg_spec}"
