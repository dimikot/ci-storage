...
# without re-raising if swallow=True. If doing is provided, it is logged on a
# success too followed with "... ok". If failure is provided, it is prepended to
# the error message.
#
def logged_result(
    *,
...
def wrap_main(main: Callable[[], Any]):
    ...
    try:
        ...
        log(f"{e}\nDoc: {e.doc if e.doc else '<empty>'}", error=True)
        sys.exit(2)
    except subprocess.CalledProcessError as e:
        log(
...
class PostJsonHttpRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def send_error(
        ...
        log(f"Error: {message} (HTTP {code})", error=True)
        if explain:
