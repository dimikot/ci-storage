...
class HandlerWebhooks:
    ...
    def handle(
        ...
        if event_payload:
            ...
            message = f"downloading {os.path.basename(path)} and parsing jobs list..."
            try:
                workflow = gh_fetch_workflow(
                    repository=repository, sha=head_sha, path=path
                )
                labels = gh_predict_workflow_labels(workflow=workflow)
                handler.log_message(
                    f"{message} " + " ".join([f"{k}:+{v}" for k, v in labels.items()])
                )
        ...
        *,
        ...
        for asg_spec in self.asg_specs:
            if asg_spec.repository == repository and asg_spec.label in labels:
                ...
                messages.append(f"{asg_spec.asg_name}:+{inc}")
        if messages:
            return handler.send_json(
                200,
                message=f"desired capacity {', '.join(messages)}"
